...
template<typename T> int LinkedList<T>::countList(T item){
    int count =0;
    current = first;
...
template<typename T> void LinkedList<T>::operator+(T item){
    addAtEnd(item);
}

template<typename T> T* LinkedList<T>::operator/(int p){
   return getAt(p);
